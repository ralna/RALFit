...
module lanczos_module
  ...
  ! Constrained version
  type, extends(params_box_type) :: params_b_type
     real(wp), dimension(:), allocatable :: t ! The m data points t_i
     real(wp), dimension(:), allocatable :: y ! The m data points y_i
  end type params_b_type
...
program lanczos
  ...
  integer :: m,n,i
  real(wp), allocatable :: x(:), blx(:), bux(:)
  type(params_type) :: params
  type(params_b_type) :: params_box
  real(wp) :: tic, toc
  ...
  print *
! stop
  ! Extend the problem to include bounds on the variables
  print *, "Constrained problem"
  x = (/ 1.2, 0.3, 5.6, 5.5, 6.5, 7.6 /) ! SP 1
 
  ! Add some box bound
  Allocate(blx(n),bux(n),params_box%t(m),params_box%y(m))

  ! Copy problem data
  params_box%y(1:m) = params%y(1:m)
  params_box%t(1:m) = params%t(1:m)
  ...
  Call nlls_setup_bounds(params_box, n, blx, bux, options, inform)
  ...
  call cpu_time(tic)
  call nlls_solve(n,m,x,eval_r, eval_J, eval_HF, params_box, options, inform)
  call cpu_time(toc)
  if(inform%status.ne.0) then
     print *, "ral_nlls() returned with error flag ", inform%status
     stop
  ...
  ! Print result
  print *, "Constrained problem (solution)"
  print *, "Objective Value at solution    = ", inform%obj
  print *, "Objective Gradient at solution = ", inform%norm_g
  print *, "Found a local optimum at x:"
  Write (*, Fmt=99998) "Idx", "blx(i)", "x(i)", "bux(i)"
  Do i = 1, n
...
99999  Format (5X,I3,2X,3(Es13.6e2,2X))
99998  Format (5X,A3,2X,3(A13,2X))
