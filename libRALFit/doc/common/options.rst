...
.. |reg_order| replace:: if ``type_of_method = 2``, the order of the regularization used (:math:`p` in  (eq::lsq)).   If ``reg_order = 0.0``, then the algorithm chooses an appropriate value of :math:`p`. 
