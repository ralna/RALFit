...
\itt{info} is an \intentinout\ scalar of type 
...
\begin{description}
   ...
   \itt{f} must be set by the routine to hold the residual function 
...
A subroutine must be supplied to calculate $HF = \sum_{i=1}^m F_i(x) \nabla^2 F_i(x)$ for a given vector $x$. It must implement the following interface:

\begin{verbatim}
abstract interface
   subroutine eval_hf_type(n, m, params, x, f, HF, status)
       integer, intent(in) :: n
       integer, intent(in) :: m
       class(params_base_type), intent(in) :: params
       double precision, dimension(n), intent(in)  :: x
       double precision, dimension(m), intent(in)  :: f
...
\begin{description}
   ...
   \itt{x} holds the current point $\iter{\vx}$ at which to evaluate $F(x)$.
  
   \itt{f} holds $F(x)$, as returned by a previous call to \texttt{eval\_F}.
