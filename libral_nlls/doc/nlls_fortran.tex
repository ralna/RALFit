...
In order to evaluate the residual, Jacobian and Hessian at a point, the user
...
\begin{description}
   ...
   \itt{r} holds $\vW\vr(x)$, the return from previous call to \texttt{eval\_r}, which will have been weighted, if appropriate, by {\tt ral\_nlls}.
...
\itt{scaled\_g} \scalarreal that holds the gradient of the objective function at the best estimate of the solution determined by the algorithm.
